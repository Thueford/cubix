...
%Die wichtigsten sind hierbei der Hit-Effekt, der beim Aufprall eines Geschosses an dessen Position einige Partikel in alle Richtungen verstreut, und der Explosions-Effekt des roten Schussmoduses.
...
Dem Folgt ein Subshader-Pass der sämtliche Rendereinstellungen vornimmt. Da Partikel transparent sind, wird die Transparent queue zum Rendern verwendet. Es ist außerdem wichtig ZWrite auszuschalten, da die Partikel sonst nicht korrekt sortiert werden (siehe \fullref{img:zoff}). Darauf folgt schließlich der tatsächliche Shader-Code des Fragment und des Vertex-Shaders.

\begin{minipage}{\linewidth}
\begin{center}
\vspace{5mm}
\captionsetup{type=figure}
\includegraphics[width=0.2\linewidth]{zwOn.png}
\captionof{figure}{ZWrite On}
\label{img:zoff}
\end{center}
\end{minipage}
